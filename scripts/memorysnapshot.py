...
OUTPUT_DIR: Path = Path(__file__).parents[1] / "profile_output"
...
def export_memory_snapshot() -> None:
    ...
    try:
        export_path = str((OUTPUT_DIR / file_prefix).with_suffix(".pickle"))
        logger.info(f"Saving snapshot to local file: {export_path}")
        torch.cuda.memory._dump_snapshot(export_path)
